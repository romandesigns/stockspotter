...
// computed detail sentences for the momentum panel. Volume is a direct
// restatement of data already on the chart (a volume sum) -- nothing to
// disagree with. Structure/wick-rejection stay grounded in the real
// backend score rather than an independent client-side re-detection, so
// the good/warning icon never disagrees with the sentence next to it.
// MA-slope detail used to be treated like volume (an independent
// client-side re-derivation) until a real live case (PPBT, 2026-09-03,
// found on web -- see that file's own header comment for the full
// story) showed the client's own quick MA9/MA20 recheck can genuinely
// disagree with the server's real maSlope factor score. maSlopeDetail
// now takes that real score's `good` verdict and only uses confident
// wording when the client's own read agrees with it.
